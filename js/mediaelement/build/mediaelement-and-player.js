	...
	mejs.MediaElementPlayer.prototype = {
		...
		init: function() {
			...
			if ((mf.isiPad && t.options.iPadUseNativeControls) || (mf.isiPhone && t.options.iPhoneUseNativeControls)) {
				...
				$('<p class="song-title"></p>').appendTo(t.controls);
		...
		buildplaypause: function(player, controls, layers, media) {
			...
			var
				...
				$('</div><div class="mejs-button mejs-playpause-button mejs-play" >' +
					'<button type="button" id="playPauseBtn" aria-controls="' + t.id + '" title="' + t.options.playpauseText + '" aria-label="' + t.options.playpauseText + '"></button>' +
				'</div>')
				.appendTo(controls)
				.click(function(e) {
					e.preventDefault();
		...
		buildduration: function(player, controls, layers, media) {
			...
			if (controls.children().last().find('.mejs-currenttime').length > 0) {
				...
				$('<div class="mejs-time mejs-duration-container">'+
					'<span class="mejs-duration">' +
						(isNaN(t.options.duration) ? 'Live' : t.options.duration > 0 ?
							mejs.Utility.secondsToTimeCode(t.options.duration, t.options.alwaysShowHours || t.media.duration > 3600, t.options.showTimecodeFrameCount,  t.options.framesPerSecond || 25) :
				   			((player.options.alwaysShowHours ? '00:' : '') + (player.options.showTimecodeFrameCount? '00:00':'00:00'))
				   		) +
					'</span>' +

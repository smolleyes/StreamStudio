...
Document.prototype._buildLinks = function() {
	...
	this._positions.forEach(function(child) {
		...
		case "clusterRelative":
			...
			offset += cp._positionTarget.getContentPosition();
			break;
		...
		if (!target || target.position !== "start") {
			console.log("Can not find target for offset=" + offset + " doc=" + self);
			return;
		}
		// console.log("Find target of offset=" + offset + " source=" + child._name
		// +
		// " to target=" + target.target._name + " type=" + target.position);
	...
	this._prepareDocument(options, function(error) {
		...
		self._computePositions(function(error) {
			...
			self._updateCRC32(function(error) {
				...
				self._write(output, source, function(error) {
					if (error) {
						return callback(error);
					}

					source.end(output, function(error) {
						if (error) {
							return callback(error);
						}

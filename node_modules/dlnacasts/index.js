  ...
  var emit = function (cst) {
    ...
    var connect = thunky(function reconnect (cb) {
      ...
      client.on('loading', function (err) {
        player._status.playerState == "TRANSITIONING"
    ...
    player.status = function (cb) {
      ...
      parallel({
        currentTime: function (acb) {
          ...
          player.client.callAction('AVTransport', 'GetPositionInfo', params, function (err, res) {
            if (err) return
            var position = {}
            position.currentTime = parseTime(res.AbsTime) | parseTime(res.RelTime)
            position.duration = hmsToSecondsOnly(res.TrackDuration)
            player.client.callAction('AVTransport', 'GetTransportInfo', params, function (err, res) {
              if (err) return
              player._status.playerState = res.CurrentTransportState
            })
            acb(null, position)
          })
        },
      ...
      function (err, results) {
        debug('%o', results)
        player._status.position = results.currentTime

...
yt.getVideoInfos = function (video_link,num,total,upnp, cb){
	if (total === 1) {
	    videos_responses = new Array();
	}
	yt.upnp = upnp;
        ...
        ytdl.getInfo(video.link, video, function(err, video, info){
            if(err) {
				...
				Iterator.iterate(info.formats).forEach(function (format,i) {
					...
					if(upnp) {
						if (in_array(resolution,res) === true || container == 'flv' || container == '3gp' || container == "webm" ) {
							return;
						}
					} else {
						if (in_array(resolution,res) === true || container == 'flv' || container == '3gp' || container == "mp4" || container == "m4a" || encoding == "VP8" ) {
							return;
						}
					}
					res.push(resolution);

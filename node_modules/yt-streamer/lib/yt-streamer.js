					...
					Iterator.iterate(info.formats).forEach(function (format,i) {
						try {
    						...
    						if(yt.upnp || yt.extPlayer) {
    							...
    							if (in_array(resolution,res) || container == 'flv' || container == '3gp' || container == "m4a" || container == "mp4" ) {
    								return;
    							}
    						}
    						res.push(resolution);
...
function getInfos(video,obj,res,settings) {
	try {
		var acode = 'bestaudio[ext=webm]/best';
		if(yt.upnp || yt.extPlayer) {
            var resol = settings.resolution.replace('p','') || '720';
            console.log('get upnp link ' + resol)
			var options = ["-f",'best[height<=?'+resol+',ext=webm]/best', '--youtube-skip-dash-manifest'];
			video.upnp = true;
			...
			var options = ['-f', obj.itag+'+'+acode, '--get-url'];
			video.upnp = false;
		}
		ytl.getInfo(video.link,options,function(err, inf){
			if(err) {
				...
				video.upnp = true;
                var resol = settings.resolution.replace('p','') || '720';
				var options = ["-f", 'best[height<=?'+res+',ext=webm]/best', '--youtube-skip-dash-manifest'];
				ytl.getInfo(video.link,options,function(err, inf){
					if(err) {
						console.log(err);

...
function getInfos(video,obj,res,settings) {
	...
	try {
		...
		if(yt.upnp || yt.extPlayer) {
			...
			var options = ["-f",'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best','--youtube-skip-dash-manifest'];
			video.upnp = true;
           ...
           var options = ["-f",'bestvideo[height<=?'+resol+',ext=webm]/best[ext=webm]','--youtube-skip-dash-manifest'];
            video.upnp = false;
        } else {

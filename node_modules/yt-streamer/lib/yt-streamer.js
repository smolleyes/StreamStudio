...
function getInfos(video,obj,res,settings) {
	try {
		...
		if(yt.upnp || yt.extPlayer) {
			...
			var options = ["-f",'best[height<=?'+resol+']/best',"--youtube-skip-dash-manifest"];
			video.upnp = true;
			...
			var options = ['-f', obj.itag+'+'+acode, '--get-url',"--youtube-skip-dash-manifest"];
			video.upnp = false;
		}
		ytl.getInfo(video.link,options,function(err, inf){
			if(err) {
				...
				video.upnp = true;
                var resol = settings.resolution.replace('p','') || '720';
				var options = ["-f", 'best[height<=?'+res+']/best',"--youtube-skip-dash-manifest"];
				ytl.getInfo(video.link,options,function(err, inf){
					if(err) {
						console.log(err);

...
function loadTable(files,$,_,dom,sdb) {
  ...
  $('#preloadTorrent').empty().remove();
  $('#fbxMsg2').remove();
  var list = __.sortBy(files, function(obj){ return obj.name; });
  $.each(list,function(i,file){
    ...
    html+='<tr><td><a href="#" class="loadStreaming" data-id="'+file.name+'">'+file.name+'</a></td><td><span><i style="display:'+viewed+';line-height: 23px;margin-right:5px;float:left;" class="glyphicon glyphicon-eye-open"></i>'+watched+'</span></td><td> '+bytesToSize(file.length,2)+'</td></tr>';
    if(i+1 == list.length){
      $('#fbxMsg2').remove();

...
dmotion.lives = function (user_search,page,filters,cb) {
    ...
    try{
       ...
       var req;
        if (user_search === '') {
            var url = 'http://'+ip.address()+':8081/?link=https://api.dailymotion.com/videos?sort=relevance&page='+page+'&limit=25&fields=onair,id,title,audience,thumbnail_240_url&filters=live';
        } else {
            var url = 'http://'+ip.address()+':8081/?link=https://api.dailymotion.com/videos?sort=relevance&page='+page+'&limit=25&search='+user_search+'&fields=onair,id,title,audience,thumbnail_240_url&filters=live';
        }
        request(''+url+'', function(err, res, body) {
            if (err) return cb(err);
            if (res.statusCode !== 200) {
...
dmotion.getVideoInfos = function (id,num,total,cb) {
    ...
    video.thumb =  obj.thumbnail_large_url;
...
function parseHtml(data, video){
    resolutions_regex = ['1080','720','480','380','240'];
    ...
    for(var v=0; v<resolutions_regex.length; v++) {
        try{
            ...
            var link= JSON.parse(decodeURIComponent(data.match(regex)[1]))
            var resolution = resolutions_string[v];
            resolutions[resolution] = [];
            resolutions[resolution]['link'] = link.url;
...
dmotion.loadSongs = function(plid,length,page,cb){
    try{
		var url = 'http://'+ip.address()+':8081/?link=https://api.dailymotion.com/playlist/'+plid+'/videos?page='+page+'&limit=25';
		request(''+url+'', function(err, res, body) {
			if (err) return cb(err);
			if (res.statusCode !== 200) {
			  return cb(new Error('Video not found'));

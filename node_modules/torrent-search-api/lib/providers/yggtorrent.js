const TorrentProvider = require('../torrent-provider');

class Yggtorrent extends TorrentProvider {
  _getScrapeDatas() {
    return {
      name: 'Yggtorrent',
      baseUrl: 'https://yggtorrent.to',
      requireAuthentification: true,
      supportCookiesAuthentification: true,
      supportCredentialsAuthentification: true,
      enableCloudFareBypass: true,
      loginUrl: '/user/login',
      loginQueryString: 'id={username}&pass={password}',
      searchUrl: '/engine/search?name={query}&description=&file=&uploader=&category=2145&subcategory=all&do=search&order=desc&sort=seed',
      currentQuery:'{query}',
      currentquerytotalResults:25,
      currentqueryTotalPages:0, 
      currentPage:1, 
      hasMore:false, 
      nextPageStartIndex:25,
      categories: {
        All: '',
        Videos: '2139,2145&subcategory=all',
        Movies: '2145&subcategory=2183',
        TV: '2145&subcategory=2184',
        Emulation: '2141',
        Games: '2142',
        Applications: '2144',
        Music: '2139',
        Books: '2140',
        GPS: '2143',
        XXX: '2188'
      },
      defaultCategory: "All",
      resultsPerPageCount: 25,
      itemsSelector: '.table tr',
      itemSelectors: [
        {
          title: 'td:nth-child(2) > a@text',
          time: 'td:nth-child(5)@text | trim',
          seeds: 'td:nth-child(8)@text | replace:"--",0 | int',
          peers: 'td:nth-child(9)@text | replace:"--",0 | int',
          size: 'td:nth-child(6)@text',
          link:'td:nth-child(2) > a@href',
          torrentId: 'td:nth-child(3) > a@target',
          desc: 'td:nth-child(2) > a@href'
        }
      ],
      paginateSelector: 'a[rel="next"]@href',
      torrentDetailsSelector: '#middle .default@html'
    };
  }

  search(query, category, limit) {
    this.skipNextLogin = true;
    return super.search(query, category, limit);
  }

  _isLogged() {
    return (
      super._isLogged() &&
      this.lastLoginTime &&
      Date.now() - this.lastLoginTime < 600000
    );
  }

  _login(cookie) {
    if (this.skipNextLogin) {
      this.skipNextLogin = false;
      return Promise.resolve();
    }
    return super._login(cookie).then((res) => {
      console.log("login res", res)
      this.lastLoginTime = Date.now();
    });
  }
}

module.exports = Yggtorrent;

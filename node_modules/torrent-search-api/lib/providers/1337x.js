...
class _1337x extends TorrentProvider {
  ...
  _downloadTorrent(torrent) {
    return Promise.fromCallback(this._x(torrent.desc, '.infohash-box span')).then(hash => {
      console.log(hash)
        return super._downloadTorrent({ link: format('http://itorrents.org/torrent/{0}.torrent', hash) });
    });
  }
}
